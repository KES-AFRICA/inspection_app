...
import '../statistics/mission_statistics.dart';
...
class _PriseTerreRepereGroup {
  final String repereName;
  final List<_PriseTerreRowItem> items;

  _PriseTerreRepereGroup({
    required this.repereName,
    required this.items,
  });
}

class _PriseTerreZoneGroup {
  final String zoneName;
  final List<_PriseTerreRepereGroup> repereGroups;
...
class _DdrEquipmentGroup {
  final String equipmentName;
  final List<_DdrRowItem> items;
...
class _DdrRepereGroup {
  final String repereName;
  final List<_DdrEquipmentGroup> equipmentGroups;
...
class _DdrZoneGroup {
  final String zoneName;
  final List<_DdrRepereGroup> repereGroups;
...
class PdfReportService {
  ...
  static pw.Widget _buildGlossaireCell(String text, bool isKey) {
    return pw.Container(
      ...
      child: pw.Text(
        text,
        style: pw.TextStyle(
          font: isKey ? _fontBold : _fontRegular,
          fontSize: 6.8,
          color: isKey ? accentColor : PdfColors.grey900,
        ),
      ),
    );
  }
  ...
  static bool _matchEquip(CoffretArmoire c, String query) {
  ...
  static bool _matchName(String name, String query) {
  ...
  static Future<_ChunkSectionResult> _addPhotosSectionChunked(
    ...
    Future<void> renderBlock(
      ...
      for (int i = 0; i < block.photos.length; i += 2) {
        ...
        final img1 = await _loadAndOptimizeImage(
          p1.filePath,
          photoContext: PdfPhotoContext.equipmentObs,
          saveFilesToDisk: saveFilesToDisk,
        );
        ...
        if (i + 1 < block.photos.length) {
          ...
          final img2 = await _loadAndOptimizeImage(
            p2.filePath,
            photoContext: PdfPhotoContext.equipmentObs,
            saveFilesToDisk: saveFilesToDisk,
          );
  ...
  static Future<_ChunkSectionResult> _addSyntheseEquipementsSectionChunked(
    ...
    Future<void> _renderEquipementsSubBatches({
      required List<_PdfEquipementItem> items,
      required String sectionKey,
...
class _PhotoSectionBlock {
  final String title;
  final String? subtitle;
  final List<_PhotoEntry> photos;
  final List<_EquipmentPhotoGroup> equipments;
  final List<_PhotoSectionBlock> subBlocks;
